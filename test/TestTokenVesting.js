...
const SingularityNetToken = artifacts.require('SingularityNetToken');
const TokenVesting = artifacts.require('TokenVesting');

contract('TokenVesting', function ([_, miner, owner, beneficiary]) {

  const amount = new BigNumber(1000);

  beforeEach(async function () {
    this.token = await SingularityNetToken.new({ from: miner });
    ...
    await this.token.transfer(this.vesting.address, amount, { from: miner });
  });

  it('cannot be released before cliff', async function () {
    await this.vesting.release(this.token.address).should.be.rejectedWith('invalid opcode');
  });
  ...
  it('should fail to be revoked by owner if revocable not set', async function () {
    ...
    await vesting.revoke(this.token.address, { from: owner }).should.be.rejectedWith(' invalid opcode');
  });
  ...
  it('should fail to be revoked a second time', async function () {
    ...
    await this.vesting.revoke(this.token.address, { from: owner }).should.be.rejectedWith('invalid opcode');
  });
